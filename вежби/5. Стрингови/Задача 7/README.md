...
Да се изврши валидација на дадената македонска регистарска таблица.
...
<button onclick="valid()">Тест</button><br>
<b><span>Излез: </span><span id="izlez"></span></b>

<script>

    function proveriBukvi (del) {
        let regexBukvi = /^[A-Z]+$/;
    ...
    function valid() {
        ...
        if (proveriBukvi(delovi[0]) && proveriBrojki(delovi[1]) && proveriBukvi(delovi[2]))
            izlezElem.innerText = "Валидна";
        else
